...
export const ProductCard = ({ product, index = 0 }) => {
  ...
  return (
    <motion.article
      ...
      onKeyDown={(e) => { if (e.target === e.currentTarget && (e.key === "Enter" || e.key === " ")) { e.preventDefault(); openProduct(product); } }}
    ...
    >
      <div className="relative aspect-square overflow-hidden bg-white">
        ...
        <span
          ...
          className={`absolute top-2.5 left-2.5 max-w-[calc(100%-60px)] truncate rounded-full px-2 py-[3px] text-[9px] sm:text-[10px] font-display font-bold tracking-wider uppercase text-white ${product.tag?.includes("OFF") ? "bg-crimson" : product.tag === "WATERPROOF" ? "bg-emeraldbadge" : product.tag === "NEW IN" ? "bg-brand" : "bg-ink"}`}
        >
          {product.tag || `${pct}% OFF`}
        </span>
        <button
          ...
          aria-pressed={liked}
          className="absolute top-1 right-1 w-11 h-11 rounded-full glass hairline flex items-center justify-center text-ink hover:scale-110 transition-transform"
        >
      ...
      <div className="p-3 sm:p-3.5">
        <div data-testid={`product-rating-${product.id}`} className="flex items-center gap-1 text-[11px] font-body text-graphite">
        ...
        <h3 data-testid={`product-title-${product.id}`} className="mt-1 font-display font-semibold text-sm text-ink leading-snug line-clamp-2 min-h-[2.6em]">{product.name}</h3>
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="w-11 h-11 rounded-full bg-ink text-white flex items-center justify-center flex-none hover:bg-brand transition-colors active:scale-95"
          >
            <Plus size={15} />
          </button>
        </div>
