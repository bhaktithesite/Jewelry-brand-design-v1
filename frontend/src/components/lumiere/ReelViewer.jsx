...
export const ReelViewer = () => {
  ...
  const next = () => setActiveReel((activeReel + 1) % REELS.length);
  const prev = () => setActiveReel((activeReel - 1 + REELS.length) % REELS.length);
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="fixed inset-0 z-[70] bg-obsidian/95 backdrop-blur-md flex items-center justify-center"
        ...
        >
          ...
          <button data-testid="reel-viewer-prev" onClick={prev} className="hidden sm:flex absolute left-6 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full glass-dark text-white items-center justify-center" aria-label="Previous reel"><ChevronLeft size={20} /></button>
          <button data-testid="reel-viewer-next" onClick={next} className="hidden sm:flex absolute right-6 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full glass-dark text-white items-center justify-center" aria-label="Next reel"><ChevronRight size={20} /></button>
