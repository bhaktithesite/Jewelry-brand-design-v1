...
const ITEMS = [
  ...
  { icon: RefreshCw, title: "Easy Returns", text: "Hassle-free 7-day exchange guarantee." },
];

export const TrustBadges = () => (
  <section data-testid="trust-badges" className="max-w-[1200px] mx-auto px-4 sm:px-6 relative z-10">
    <div className="flex sm:grid sm:grid-cols-3 gap-3 overflow-x-auto no-scrollbar snap-x snap-mandatory -mx-4 px-4 sm:mx-0 sm:px-0">
      {ITEMS.map((it, i) => (
        <Reveal key={it.title} delay={i * 0.08} className="snap-start min-w-[244px] sm:min-w-0">
          <div data-testid={`trust-badge-${i}`} className="bg-[#FFFCF4] hairline rounded-2xl p-3 sm:p-4 flex items-start gap-3 h-full">
