...
export const FilterTabs = () => {
  ...
  return (
    <div data-testid="filter-tabs" className="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        {FILTER_TABS.map((t) => {
          ...
          return (
            <button
              ...
              onClick={() => pick(t.key)}
              className={`relative flex-none rounded-full px-4 h-9 font-display text-xs font-semibold whitespace-nowrap transition-colors ${active ? "text-white" : "bg-subtle text-ink hover:bg-[#E2EAF3]"}`}
              aria-pressed={active}
            >
              {active && (
      ...
      <div className="hidden sm:flex items-center gap-2 bg-surface hairline rounded-full px-4 h-10 w-64">
        ...
        />
        {query && <button data-testid="catalog-search-clear" onClick={() => setQuery("")} className="text-graphite hover:text-ink"><X size={14} /></button>}
      </div>
    </div>
  );
};
