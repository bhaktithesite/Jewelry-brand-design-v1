...
const WishItem = ({ product }) => {
  ...
  return (
    ...
    >
      <button onClick={view} className="w-20 h-20 rounded-xl overflow-hidden bg-subtle flex-none hairline">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2 flex items-center gap-2">
          <button
            ...
            className="h-8 px-3 rounded-full bg-ink text-white font-display text-xs font-bold inline-flex items-center gap-1.5 hover:bg-brand transition-colors"
          >
            <ShoppingBag size={13} /> Add to Bag
          </button>
          <button data-testid={`wishlist-view-${product.id}`} onClick={view} className="h-8 px-3 rounded-full bg-subtle text-ink font-display text-xs font-semibold inline-flex items-center gap-1.5">
            <Eye size={13} /> View
          </button>
        </div>
      </div>
      <button data-testid={`wishlist-remove-${product.id}`} onClick={() => toggleWishlist(product.id)} className="self-start text-crimson hover:scale-110 transition-transform" aria-label="Remove from wishlist">
...
export const WishlistDrawer = () => {
  ...
  useEffect(() => {
    const onKey = (e) => e.key === "Escape" && setWishlistOpen(false);
    ...
    <AnimatePresence>
      {wishlistOpen && (
        <motion.div className="fixed inset-0 z-[60] flex items-end sm:items-stretch sm:justify-end" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <div data-testid="wishlist-drawer-backdrop" className="absolute inset-0 bg-obsidian/45 backdrop-blur-sm" onClick={() => setWishlistOpen(false)} />
          <motion.aside
          ...
          >
            <div className="flex items-center justify-between px-5 sm:px-6 h-16 border-b border-[var(--border-light)]">
              <div>
                <p className="font-body text-[11px] tracking-[0.25em] uppercase text-graphite">Saved Hearts</p>
              ...
              <button data-testid="wishlist-close-btn" onClick={() => setWishlistOpen(false)} className="w-9 h-9 rounded-full bg-subtle flex items-center justify-center text-ink"><X size={16} /></button>
            </div>
            ...
            {items.length === 0 ? (
              ...
              <>
                ...
                <div className="flex-none border-t border-[var(--border-light)] px-5 sm:px-6 py-4 pb-safe bg-surface">
                  <button
